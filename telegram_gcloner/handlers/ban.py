...
def ban(update: Update, context: CallbackContext):
    ...
    if not context.bot_data.get('ban', None):
        ...
        update.message.reply_text('Already exists in the blacklist.')
        return
    ...
    if tasks:
        for t in tasks:
            ...
            break
    update.message.reply_text('Has been added to the blacklist.')
...
def unban(update: Update, context: CallbackContext):
    ...
    if user_id in context.bot_data.get('ban', []):
        ...
        context.bot_data['ban'] = new_ban
        context.dispatcher.update_persistence()
        update.message.reply_text('Has been removed from the blacklist.')
